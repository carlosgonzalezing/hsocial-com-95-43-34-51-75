...
export function CommentFooter({ 
  ...
  const validReactionTypes: ReactionType[] = ["like", "love"];
  
  if (userReaction && !validReactionTypes.includes(userReaction as ReactionType)) {
    safeUserReaction = null;
  }
  
  // Handle reaction click - we're limiting to just "like" for comments
  const handleReaction = (id: string) => {
    // Pass a literal "like" string to ensure type safety
    onReaction(id, "like");
  };
